...
export default class Project {
            ...
            this.#connection.query(query, async (error, projects) => {
                if (!error) {
                    ...
                    for (let project of projects) {
                        project.boardCount = (await this.getProjectBoards(project.id)).length
        ...
        return new Promise((resolve, reject) => {
            ...
            this.#connection.query(query, async (error, result) => {
                if (!error) {
                    ...
                    const unprocessedBoards = await this.getProjectBoards(project.id);
                    const boards = []
                    ...
                    resolve({ id: project.id, name: project.name, boards: boards, boardCount: unprocessedBoards.length, dateCreated: project.date_created });
    ...
    getCards(boardId) {
        return new Promise((resolve, reject) => {
            const query = `SELECT * FROM card WHERE board_id = ${boardId}`;
    ...
    addProject(projectId, projectName) {
        return new Promise((resolve, reject) => {
            let query = `INSERT INTO project (id, name) VALUES ('${projectId}' , '${projectName}')`;
            this.#connection.query(query, (error, result) => {
                if (!error) {
                    ...
                    reject({ message: "failed to add project to database", error })
                }
            })
        })
    }
    ...
    deleteProject(projectId) {
        ...
        return new Promise((resolve, reject) => {
            let query = `SELECT id FROM board WHERE project_id = ${projectId}`
            this.#connection.query(query, async (error, boardList) => {
                if (!error) {
                    boardList.forEach(async board => {
                        const message = await this.deleteCardsOfBoard(board.id)
                        if (message == this.FAILED) {
                            reject(this.FAILED)
                        }
                    })

                    const message = await this.deleteBoardsOfProject(projectId)
                    if (message == this.FAILED) {
                        reject(this.FAILED)
                    } else {
                        const message = await this.deleteEmptyProject(projectId)
                        if (message == this.FAILED) {
                            reject(this.FAILED)
                        } else {
    ...
    deleteBoardsOfProject(projectId) {
        return new Promise((resolve, reject) => {
            const query = `DELETE FROM board WHERE project_id = ${projectId}`
            this.#connection.query(query, (error, result) => {
                if (!error) {
                    resolve(this.SUCCESS)
                } else {
